...
Betrachte einen **Baum** aus $N$ **Knoten**, nummeriert von $0$ bis $N-1$. Knoten $0$ heisst **Wurzel**.
...
- jeder Knoten, dessen Grosselternknoten $i$ ist, und
- jeder Knoten, dessen Urgrosselternknoten $i$ ist,
- etc.
...
die jeweils durch ein Paar von Ganzzahlen $(L, R)$ bestimmt werden, beantwortet.
Die Antwort auf die Anfrage sollte wie folgt berechnet werden:

Stellen wir uns vor, dass wir jedem Knoten des Baumes eine Ganzzahl,
...
ist nicht kleiner als $L$ und nicht grösser als $R$.
...
Die **Gesamtkosten** schliesslich sind die Summe der Kosten aller Knoten.
...
* $P$, $W$: Arrays der Länge $N$ von Ganzzahlen, die die Elternknoten und Gewichte angeben.
...
* $L$, $R$: Die Ganzzahlen, die die Anfrage beschreiben.
...
das heisst, dass die Summe der Koeffizienten in jedem Teilbaum gleich $1$ sein müssen.
Betrachte die Koeffizientenfolge $[-1, 1, 1]$.
Der Baum und die entsprechenden Koeffizienten (in grau schattierten Rechtecken) sind hier dargestellt:
...
Beachte, dass die zweite Zeile der Eingabe **lediglich $\mathbf{N-1}$ Ganzzahlen** enthält, da der Beispielgrader den Wert von $P[0]$ nicht einliest.
